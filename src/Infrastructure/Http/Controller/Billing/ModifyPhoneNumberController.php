...
use Konair\HAP\Payment\Application\Command\Billing\ModifyPhoneNumber\ModifyPhoneNumberRequest;
use Konair\HAP\Payment\Application\Command\Billing\ModifyPhoneNumber\ModifyPhoneNumberService;
...
final class ModifyPhoneNumberController
{
    ...
    public function __invoke(string|null $billingDataId, Request $httpRequest): Response
    {
        ...
        $request = new ModifyPhoneNumberRequest(
            $billingDataId,
            $httpRequest->get('phoneNumber'),
        );
        $service = new ModifyPhoneNumberService($this->repository);
