...
use Konair\HAP\Payment\Application\Command\Billing\ModifyFirmName\ModifyFirmNameRequest;
use Konair\HAP\Payment\Application\Command\Billing\ModifyFirmName\ModifyFirmNameService;
...
final class ModifyFirmNameController
{
    ...
    public function __invoke(string|null $billingDataId, Request $httpRequest): Response
    {
        ...
        $request = new ModifyFirmNameRequest(
            $billingDataId,
            $httpRequest->get('firmName'),
        );
        $service = new ModifyFirmNameService($this->repository);
