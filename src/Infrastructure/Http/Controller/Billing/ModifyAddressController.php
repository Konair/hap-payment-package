...
use Konair\HAP\Payment\Application\Command\Billing\ModifyAddress\ModifyAddressRequest;
use Konair\HAP\Payment\Application\Command\Billing\ModifyAddress\ModifyAddressService;
...
final class ModifyAddressController
{
    ...
    {
        if (is_null($billingDataId) || is_null($httpRequest->get('address'))) {
            return new JsonResponse([], Response::HTTP_NOT_FOUND);
        }

        $request = new ModifyAddressRequest(
            $billingDataId,
            $httpRequest->get('address')['country'] ?? null,
            $httpRequest->get('address')['zip'] ?? null,
            $httpRequest->get('address')['city'] ?? null,
            $httpRequest->get('address')['line'] ?? null,
        );
        $service = new ModifyAddressService($this->repository);
