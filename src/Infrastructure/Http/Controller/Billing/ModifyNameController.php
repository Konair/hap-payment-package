...
use Konair\HAP\Payment\Application\Command\Billing\ModifyName\ModifyNameRequest;
use Konair\HAP\Payment\Application\Command\Billing\ModifyName\ModifyNameService;
...
final class ModifyNameController
{
    ...
    public function __invoke(string|null $billingDataId, Request $httpRequest): Response
    {
        ...
        $request = new ModifyNameRequest(
            $billingDataId,
            $httpRequest->get('firstName'),
            $httpRequest->get('lastName'),
        );
        $service = new ModifyNameService($this->repository);
